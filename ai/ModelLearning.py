...
def read_data(filename):
    with open(filename, 'r', encoding='UTF-8') as f: # 한글이 포함되어있을때 UTF-8작성
...
# 사용하여 간단한 문장분석을 실행한다.
# JPype1은 반드시 1미만의 버전 사용
# pip uninstall JPype1
# pip install "jpype1<1"

okt = Okt()
# print(okt.pos('이 밤 그날의 반딧불을 당신의 창 가까이 보게요'))
...
# 한글폰트 설정을 해줘야 깨지지 않고 출력
# font_name = 'C:\Windows\Fonts\gulim.ttc' #gulim.ttc
# font_name = font_manager.FontProperties(fname=font_name).get_name()
# rc('font', family=font_name)
#
# plt.figure(figsize=(20,10)) # 그래프 사이즈
# text.plot(50)               # 그래프 그리기
# plt.show
# seaborn => 다른 그래프 그리는 라이브러리


# 중복 단어 48920개 => 인공지능 학습
# 그 중 빈도 가장 높은 토큰 5000개를 사용하여 데이터를 벡터화
...
# Train(학습) 데이터 정의
train_x = [term_frequency(d) for d, _ in train_docs] # 데이터   # _ => 이 값은 안쓰겠다. 오직 튜플 언패키징 위해 사용
train_y = [c for _, c in train_docs]                 # 긍부정
# Test(검증) 데이터 정의
test_x = [term_frequency(d) for d, _ in test_docs]
test_y = [c for _, c in test_docs]

# 이제 데이터를 float형으로 변환 시켜주면 데이터 전처리 과정 끝
print('>> 형변환 시작')
